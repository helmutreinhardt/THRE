#define FW_VERSION 364    //  Firmware Version  
#define ledPIN 27         //  Digital Pin WS2812 bzw. NeoPixel
...
const char* GITHUB_TOKEN = "bbc0df7596a44e1b9cc134505262905";
...
bool LoadFileFromTokio(String url){
  if (PROM.online == 0) return false;
  ...
  // Daten direkt aus dem HTTP-Stream lesen
  WiFiClient *stream = http.getStreamPtr();
  Filebufferln = stream->readBytes(Filebuffer, sizeof(Filebuffer));
  Serial.println("File erfolgreich geladen.");
  Serial.println(Filebufferln);
  return true;
}

void LoadAndShowFromTokio(String nr){
  String url="https://raw.githubusercontent.com/helmutreinhardt/THRE/main/bilder/bild"+nr+".bin";
  if(!LoadFileFromTokio(url)) return;
  Anzeige363(Filebuffer);
}

void LoadHelpFromTokio( String nr) {
  String url="https://raw.githubusercontent.com/helmutreinhardt/THRE/main/helps/help"+nr+".txt";
  if(!LoadFileFromTokio(url)) return;
  size_t len = min((size_t)Filebufferln, sizeof(Ausgabebuffer) - 1);
  memcpy(Ausgabebuffer, Filebuffer, len);
  Ausgabebuffer[len] = '\0';
  Serial.println(Ausgabebuffer);
}
String VersionFromTokio() {
  String antwort;
  String url="https://raw.githubusercontent.com/helmutreinhardt/THRE/main/version.txt";
  if(!LoadFileFromTokio(url)) return "0000";
  memcpy(Ausgabebuffer, Filebuffer, 4);
  Ausgabebuffer[4] = '\0';
  Serial.println(Ausgabebuffer);
  antwort=Ausgabebuffer;
  return antwort;
}
...
void OTAEnable() {
  int vr = FW_VERSION;
  String v = VersionFromTokio();
  int vs = v.toInt();
  if (vs==0 or PROM.online == 0){PlayMP3(240);return;}
  if (vs > vr) {
    PROM.otaEnable = 1;
...
bool SETnickname(String na) {
  ...
  if (p1 == "HOME") {
    ...
    ServerSave = false;
    scroll = false;
    LoadAndShowFromTokio(p2);
  } else if (p1 == "HELP") {
    ServerSave = false;
    scroll = false;
    LoadHelpFromTokio(p2);
  } else if (p1 == "MP3") {
    ServerSave = false;
    scroll = false;
    PlayMP3(intp2,0);
...
void Info() {
  ...
  htmlRumpf+=RTL("Serverversion",String(vv));
  PlayMP3(250, 0);
}
void QONLINE() {
  String onl = "ONLINE?\n";
...
int SetPixel(int nr) {
  ...
  if (ww > 0) {
    String w = Worte[nr] + " ";
    if (PROM.printDetail == 1) {Serial.print(w);}
    ret = nr + 1;
  }
  return ret;
}
  ...
  for (int z = 0; z < Anz_Feiertage; z++) {
    ...
    if (f != -1) {
      ...
      if (diff >= -null and diff <= null) {
        ...
        LEDFTMerker[f] = 1; 
        ft = true;
        if (diff==0) {Holiday_Name=FN[z];Serial.println(Holiday_Name);}
...
void AnzeigeFTName(String StundeMinute){
  int st = StundeMinute.substring(0, 2).toInt();
  int mi = StundeMinute.substring(2, 4).toInt()+1;    // +1
  if ((mi%5)!=0 or Holiday_Name == "") return;                              // durch +1: 4;9;14; ...
      Holiday_Name.toCharArray(Ausgabebuffer, sizeof(Ausgabebuffer));
      scroll(Ausgabebuffer);  // Text und Geschwindigkeit (ms pro Schritt) mit Farbwechsel
}
...
bool otaFirmware(String p2,String p3) {
  ...
  if (p2 == "TOKIO") {
    if(p3 =="AKIHABARA"){
      OHNEotaUpdate("https://raw.githubusercontent.com/helmutreinhardt/THRE/main/newest/firmwareV4.bin");
    }
    else{ OHNEotaUpdate("https://raw.githubusercontent.com/helmutreinhardt/THRE/main/firmwareV3.bin");}
  }
  else if (p2 == "NOKEY") {
...
void startOTA() {
  ...
  int nr;
  ArduinoOTA.begin();
  PlayMP3(146, 0, 4500);  // UpdateFenster geöffnet
...
void setup() {
  ...
  ipString = nick +"="+ ip.toString() + " ";  // Name der Uhr + IP
  SetSommerWinter();
  LEDCounter(3, 0);
  scroll(ipString);
  last5Minuten = 0;
  ...
  if (reason == ESP_RST_BROWNOUT) {
    Serial.println("⚠️ Spannungsersorgung schlecht?!");
  } else {
    Serial.print("Reset-Grund: ");
    Serial.println(reason);
  }
  LoadAndShowFromTokio("0");
  connectPush();
}
//****************************************************************
